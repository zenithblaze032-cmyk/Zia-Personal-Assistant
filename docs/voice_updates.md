# How to Change Zia's Voice (Piper TTS)

Zia now uses a **config-driven** Piper TTS system. You can switch between male and
...
3. Restart Zia (`run.bat`). The new voice downloads automatically and starts speaking.

## Recommended Voices (curated Top 5 male & female)

| #   | Male voices                          | Character                                     |     | #                                   | Female voices                       | Character |
| --- | ------------------------------------ | --------------------------------------------- | --- | ----------------------------------- | ----------------------------------- | --------- |
| 1   | `en_GB-alan-medium` ★ default        | Deep British butler — classic Zia             | 1   | `en_GB-jenny_dioco-medium`          | Refined RP British — "Pepper Potts" |
...
venv\Scripts\python.exe -c "from core.piper_tts import ZiaTTS; ZiaTTS('en_US-ryan-high').speak('Testing one two three.')"
```

## Using Any Other Piper Voice (not in the catalog)

Zia isn't limited to the 10 curated voices — **any** voice from the official Piper
...
```env
Zia_PIPER_VOICE=en_GB-alba-medium
```
...
| Still hearing the old voice  | Restart Zia — the voice is loaded once at startup.                      |
