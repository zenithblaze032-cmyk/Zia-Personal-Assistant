...
# We use the standard openai client for Cerebras, Groq (optional, but they have their own), OpenRouter, Nvidia
try:
...
def call_openai_compatible(messages, use_tools, base_url, api_key, model_name):
    ...
    for tool_call in message.tool_calls:
        ...
        if tool_func:
            log.info(f"Zen Mode Executing tool: {tool_name} with args: {tool_args}")
            try:
...
def generate_zen_chat(messages: List[Dict[str, Any]], use_tools: bool = True) -> str:
    ...
    Fallback order: Cerebras -> Groq -> Gemini Flash -> OpenRouter -> Nvidia -> Local.
    """
    import datetime
    ...
    log.error("Zen Mode: All fast APIs failed or limits reached. Falling back to local.")
    st.zen_mode = False
