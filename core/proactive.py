...
from core.asr import is_user_speaking
from core.context import Context

log = logging.getLogger(__name__)

def _monitoring_loop(ctx: Context):
    """
...
def start_proactive_monitoring(ctx: Context):
    """Starts the background thread for system monitoring."""
    t = threading.Thread(target=_monitoring_loop, args=(ctx,), daemon=True)
    t.start()
